...
void get_kmer_signal_per_numerized_sequence_signal(char* seq_signal, int l_signal, double* kmer_signal, int k)
{
	...
	int cur_val = -1;
	int win_end_base = 1;
	for (int i_nuc = l_half_k+1; i_nuc <= l_signal-l_half_k-1; i_nuc++)
	{
		...
		if (cur_val == -1)
		{
			...
			win_end_base = cur_base / 4;
		}
		else
		{
			// Remove the previous windows' starting value.
			int prev_win_start = i_nuc - l_half_k - 1;
			cur_val -= nuc_2_num(seq_signal[prev_win_start]);
			
			// Divide by 4.
			cur_val /= 4;

			// Add the current window's value.
			int cur_win_end = MIN(i_nuc + l_half_k, l_signal - 1);
			cur_val += (nuc_2_num(seq_signal[cur_win_end]) * win_end_base);
		}
		...
		if (check_val)
		{
			...
			{
				//fprintf(stderr, "%d ", numerized_seq_signal[j_nuc]);
				test_cur_val += nuc_2_num(seq_signal[j_nuc]) * cur_base;
				cur_base *= 4;
			} // j_nuc loop.

			if (cur_val != test_cur_val)
			{
				fprintf(stderr, "Could not match the current value %d: %d, %d (%d)\n", i_nuc, cur_val, test_cur_val, win_end_base);
				exit(0);
			}
		}
	...
	while(1)
	{
		...
		if(cur_line == NULL)
		{
			...
			if(cur_entry_i > 1)
			{
				...
				if (t_string::string_length(cur_entry_id) > 50)
				{
					char* copied_id = t_string::substring(cur_entry_id, 0, 50);
					sprintf(cur_entry_id, "%s_%d", copied_id, n_seqs);
					delete[] copied_id;
				}
		...
		else if(cur_line[0] == '>')
		{
			if(cur_entry_i > 1)
			{
				...
				// Update the counter at each file dumping. First will be called 0.
				n_seqs++;
			}
...
if(_DUMP_GENOME_SEQUENCE_MESSAGES_)
				...
				{
if(_DUMP_GENOME_SEQUENCE_MESSAGES_)
					fprintf(stderr, "Not Pass: %s\t%s\t%lf\n", valid_raw_enriched_motifs->at(i_kmer)->kmer, valid_raw_enriched_motifs->at(j_kmer)->kmer, cur_kmers_match_score);
				}
				//getc(stdin);
			}
		} // j_kmer loop.
