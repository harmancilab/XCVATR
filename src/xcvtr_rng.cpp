...
// Generates a random value from standard normal distribution using Box-Muller transform.
double t_rng::random_gaussian_double_ran3()
{
	...
	U1 = this->random_double_ran3();
	U2 = this->random_double_ran3();

	U1 = (U1==0)?(U1+=1.0/MBIG):(U1);
	U2 = (U2==0)?(U2+=1.0/MBIG):(U2);

	double cur_gaussian_rand = sqrt(-2 * log(U1)) * cos(2*my_PI*U2);

	return(cur_gaussian_rand);
}
