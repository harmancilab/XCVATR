...
void convert_ENCODE2_TF_peak_narrowPeak_2_GFF(char* narrowPeak_fp, char* source_str, char* TF_name, char* op_gff_fp)
{
	vector<t_annot_region*>* np_regs = load_narrowPeak(narrowPeak_fp);
	fprintf(stderr, "Loaded %d narrowPeak regions.\n", np_regs->size());

	FILE* f_gff_op = open_f(op_gff_fp, "w");
	for (int i_reg = 0; i_reg < (int)(np_regs->size()); i_reg++)
	{
		fprintf(f_gff_op, "%s\t%s\tENCODE2_TF_Peak\t%d\t%d\t.\t%c\t.\tTF_name=%s;\n",
			np_regs->at(i_reg)->chrom,
			source_str,
			np_regs->at(i_reg)->start,
			np_regs->at(i_reg)->end,
			np_regs->at(i_reg)->strand,
			TF_name);
	} // i_reg loop.
	fclose(f_gff_op);
}

vector<t_annot_region*>* load_Regions_as_Interval(char* feats_bed_fp)
{
	fprintf(stderr, "Loading features as Intervals from %s\n", feats_bed_fp);
	if (t_string::ends_with(feats_bed_fp, "interval"))
	{
		vector<t_annot_region*>* feat_regs_w_intervals = load_Interval(feats_bed_fp);

		return(feat_regs_w_intervals);
	}
	if (t_string::ends_with(feats_bed_fp, "bed12"))
	{
		vector<t_annot_region*>* feat_regs_w_intervals = load_BED12(feats_bed_fp);

		return(feat_regs_w_intervals);
	}
	else if (t_string::ends_with(feats_bed_fp, "bed"))
	{
		vector<t_annot_region*>* feat_regs_w_intervals = load_BED(feats_bed_fp);

		for (int i_reg = 0; i_reg < feat_regs_w_intervals->size(); i_reg++)
		{
			vector<t_annot_region*>* cur_reg_intervals = new vector<t_annot_region*>();
			cur_reg_intervals->push_back(duplicate_region(feat_regs_w_intervals->at(i_reg)));
			feat_regs_w_intervals->at(i_reg)->intervals = cur_reg_intervals;
		} // i_reg loop.

		return(feat_regs_w_intervals);
	}

	fprintf(stderr, "Could not understand the file extension.\n");
	exit(0);
	return(NULL);
}

void extract_promoter_regions_per_GFF(char* config_fp, char* argv[], int argc, char* gff_fp, char* op_fp)
{
	...
	int promoter_margin_l = 0;
	char promoter_margin_l_str[1000];
	if (get_cli_value(config, cli, "promoter_margin_l", "-promoter_margin_l", promoter_margin_l_str, true))
	{
		promoter_margin_l = atoi(promoter_margin_l_str);
	}
	else
	{
		fprintf(stderr, "Could not read: promoter_margin_l");
		exit(0);
	}

	FILE* f_op = open_f(op_fp, "w");

	vector<t_annot_region*>* gff_regs = load_GFF(gff_fp);
	fprintf(stderr, "Loaded %d gff regions.\n", gff_regs->size());

	for (int i_reg = 0; i_reg < gff_regs->size(); i_reg++)
	{
		// Parse the gff line.
		t_gff_info* cur_reg_info = (t_gff_info*)(gff_regs->at(i_reg)->data);
		char* grp_str = cur_reg_info->group_str;

		int i_chrom = t_string::get_i_str(chr_ids, gff_regs->at(i_reg)->chrom);
		if (i_chrom == chr_ids->size())
		{
			continue;
		}

		int l_chrom = chr_lengths->at(i_chrom);

		t_string_tokens* toks = t_string::tokenize_by_chars(grp_str, ";");

		char gene_name[1000];
		strcpy(gene_name, "NONAME");
		char gene_id[1000];
		strcpy(gene_id, "NOID");
		char gene_type[1000];
		strcpy(gene_type, "NOTYPE");

		for (int i_tok = 0; i_tok < toks->size(); i_tok++)
		{
			if (t_string::starts_with(toks->at(i_tok)->str(), "gene_id="))
			{
				t_string_tokens* cur_id_toks = toks->at(i_tok)->tokenize_by_chars("=");
				strcpy(gene_id, cur_id_toks->at(1)->str());
				t_string::clean_tokens(cur_id_toks);
			}

			if (t_string::starts_with(toks->at(i_tok)->str(), "gene_name="))
			{
				t_string_tokens* cur_name_toks = toks->at(i_tok)->tokenize_by_chars("=");
				strcpy(gene_name, cur_name_toks->at(1)->str());
				t_string::clean_tokens(cur_name_toks);
			}

			if (t_string::starts_with(toks->at(i_tok)->str(), "gene_type="))
			{
				t_string_tokens* cur_type_toks = toks->at(i_tok)->tokenize_by_chars("=");
				strcpy(gene_type, cur_type_toks->at(1)->str());
				t_string::clean_tokens(cur_type_toks);
			}
		} // i_tok loop.

		  // Dump the promoters vicinity.
		int tss = gff_regs->at(i_reg)->start;
		if (gff_regs->at(i_reg)->strand == '-' ||
			gff_regs->at(i_reg)->strand == 'R')
		{
			tss = gff_regs->at(i_reg)->end;
		}

		int prom_start = MAX(1, tss - promoter_margin_l);
		int prom_end = MIN(l_chrom - 10, tss + promoter_margin_l);

		fprintf(f_op, "%s\t%d\t%d\t%s %s %s\t.\t%c\n", gff_regs->at(i_reg)->chrom,
			translate_coord(prom_start, CODEBASE_COORDS::start_base, BED_COORDS::start_base),
			translate_coord(prom_end, CODEBASE_COORDS::end_base, BED_COORDS::end_base),
			gene_name, gene_id, gene_type,
			gff_regs->at(i_reg)->strand);
...
vector<t_annot_region*>* get_non_overlapping_top_N_regions(char* sorted_bed_fp, int n_regs_2_select, int i_col_2_sort_with, int min_pw_reg_dist, char* op_fp)
{
	...
	while (cand_reg_i < regs_w_lines->size() &&
			non_overlapping_regions->size() < n_regs_2_select)
	{
		// Go over each region and select.
		double min_cur_reg_2_ol_regs_dist = -1;
		for (int i_reg = 0; i_reg < non_overlapping_regions->size(); i_reg++)
		{
	...
	cand_reg_i = regs_w_lines->size() - 1;
	while (cand_reg_i >= 0 &&
		non_overlapping_regions->size() < 2*n_regs_2_select)
	{
	...
	vector<double>* rand_overlap_covgs)
{
	...
	vector<t_annot_region*>** reg1_rand_regions_per_randomization = generate_random_regions_per_reference_regions_buffered(reg1, rng, chr_ids, chr_ls, n_rands);
	vector<t_annot_region*>** reg2_rand_regions_per_randomization = generate_random_regions_per_reference_regions_buffered(reg2, rng, chr_ids, chr_ls, n_rands);
	fprintf(stderr, "Generated randomized regions.\n");

	// Do all the randomizations, process.
	for(int i_rand = 0; i_rand < n_rands; i_rand++)
	{

		fprintf(stderr, "Processing %d. randomization.                 \r", i_rand);
		vector<t_annot_region*>* cur_rand_reg1 = reg1_rand_regions_per_randomization[i_rand];
		vector<t_annot_region*>* cur_rand_reg2 = reg2_rand_regions_per_randomization[i_rand];

		// Merge the current regions.
		vector<t_annot_region*>* cur_rand_merged_reg1 = merge_annot_regions(cur_rand_reg1, 1);
		vector<t_annot_region*>* cur_rand_merged_reg2 = merge_annot_regions(cur_rand_reg2, 1);

		// Get the overlap statistics.
		//vector<t_annot_region*>* cur_overlaps = intersect_annot_regions(cur_rand_reg1, cur_rand_reg2, true);
		vector<t_annot_region*>* cur_overlaps = intersect_annot_regions(cur_rand_merged_reg1, cur_rand_merged_reg2, true);
		double cur_overlap_covg = 0;
		for(int i_int = 0; i_int < (int)cur_overlaps->size(); i_int++)
		{
			t_intersect_info* cur_int_info = (t_intersect_info*)(cur_overlaps->at(i_int)->data);
			cur_overlap_covg += cur_int_info->l_overlap;
			delete cur_int_info;
		} // i_int loop.
		rand_overlap_covgs->push_back(cur_overlap_covg);			

		// Free memory.
		delete_annot_regions(cur_overlaps);
		delete_annot_regions(cur_rand_reg1);
		delete_annot_regions(cur_rand_reg2);

		delete_annot_regions(cur_rand_merged_reg1);
		delete_annot_regions(cur_rand_merged_reg2);
	} // i_rand loop.

	// Free memory.
	delete [] reg1_rand_regions_per_randomization;
	delete [] reg2_rand_regions_per_randomization;

	// Original overlaps: Merge the regions first, then take the overlaps.
	vector<t_annot_region*>* merged_reg1 = merge_annot_regions(reg1, 1);
	vector<t_annot_region*>* merged_reg2 = merge_annot_regions(reg2, 1);
	vector<t_annot_region*>* orig_overlaps = intersect_annot_regions(merged_reg1, merged_reg2, true);
	//vector<t_annot_region*>* orig_overlaps = intersect_annot_regions(reg1, reg2, true);
	orig_overlap_covg = 0;
	for(int i_int = 0; i_int < (int)orig_overlaps->size(); i_int++)
	{
		t_intersect_info* cur_int_info = (t_intersect_info*)(orig_overlaps->at(i_int)->data);
		orig_overlap_covg += cur_int_info->l_overlap;
		delete cur_int_info;
	} // i_int loop.

	delete_annot_regions(orig_overlaps);
	delete_annot_regions(merged_reg1);
	delete_annot_regions(merged_reg2);
}
...
{
	vector<t_annot_region*>** random_regions_per_randomization = new vector<t_annot_region*>*[n_rands+2];
	//int n_regions = (int)reference_regs->size();
	for(int i_rand = 0; i_rand < n_rands; i_rand++)
	{
		random_regions_per_randomization[i_rand] = new vector<t_annot_region*>();
	} // i_rand loop.

	// Process each chromosome separately.
	for(int i_chr = 0; i_chr < (int)chr_ids->size(); i_chr++)
	{		
		vector<t_annot_region*>* cur_chr_ref_regs = get_regions_per_chromosome(reference_regs, chr_ids->at(i_chr));
		fprintf(stderr, "Generating %ld randomized regions on %s (%d)\n", cur_chr_ref_regs->size(), chr_ids->at(i_chr), l_per_chr_id->at(i_chr));

		// Go over each regions and generate a random region on the current chromosome.
		for(int i_rand = 0; i_rand < n_rands; i_rand++)
		{
			vector<t_annot_region*>* cur_cur_cur_rand_random_regions = new vector<t_annot_region*>();
			for(int i_reg = 0; i_reg < (int)cur_chr_ref_regs->size(); i_reg++)
			{
				int l_cur_seq = cur_chr_ref_regs->at(i_reg)->end - cur_chr_ref_regs->at(i_reg)->start;

				// Choose the strand.
				int strand_i = (int)(floor(rng->random_double_ran3() * 2));
				char strand_chars[] = "+-";

				// choose indices.
				int cur_reg_start = (int)(floor(rng->random_double_ran3() * (l_per_chr_id->at(i_chr)-1-l_cur_seq)));
				int cur_reg_end = cur_reg_start + l_cur_seq;

				//if(i_reg < 10)
				//{
				//	fprintf(stderr, "%s\t%d\t%d\n", chr_ids->at(i_chr), cur_reg_start, cur_reg_end);
				//}

				t_annot_region* cur_rand_reg = get_empty_region();
				cur_rand_reg->chrom = t_string::copy_me_str(chr_ids->at(i_chr));
				cur_rand_reg->start = cur_reg_start;
				cur_rand_reg->end = cur_reg_end;
				cur_rand_reg->score = cur_chr_ref_regs->at(i_reg)->score;
				cur_rand_reg->strand = strand_chars[strand_i];

				cur_cur_cur_rand_random_regions->push_back(cur_rand_reg);
			} // i_reg loop.

			// Add the current random region list to the end of all randomized regions.
			random_regions_per_randomization[i_rand]->insert(random_regions_per_randomization[i_rand]->end(), 
				cur_cur_cur_rand_random_regions->begin(), cur_cur_cur_rand_random_regions->end());
			delete cur_cur_cur_rand_random_regions;
		} // i_rand loop.

		delete cur_chr_ref_regs;
	} // i_chr loop.

	return(random_regions_per_randomization);
}
...
{
	vector<t_annot_region*>* random_regions = new vector<t_annot_region*>();
	//int n_regions = (int)reference_regs->size();
	...
	for(int i_chr = 0; i_chr < (int)chr_ids->size(); i_chr++)
	{
		...
		{
			// choose chromosome.
			int i_chr = (int)(floor(rng->random_double_ran3() * chr_ids->size()));
			...
			random_regions->push_back(cur_rand_reg);
		} // i_reg loop.
...
// This is the function that shuffles the id's between multitrack profiles for the regions.
...
{
	// Pool all the regions.
	vector<t_annot_region*>* pooled_regions = new vector<t_annot_region*>();
	for(int i_p = 0; i_p < (int)regions_per_profile->size(); i_p++)
	{
		pooled_regions->insert(pooled_regions->end(), regions_per_profile->at(i_p)->begin(), regions_per_profile->at(i_p)->end());
	} // i_p loop.

	// Allocate the shuffled regions: One shuffling generates a list of regions for each profile.
	vector<vector<t_annot_region*>*>** shuffled_merged_regions_per_profile_per_shuff = new vector<vector<t_annot_region*>*>*[n_shufflings + 2];

	t_rng* rng = new t_rng(t_seed_manager::seed_me());

	// Go over all the shufflings.
	for(int i_shuff = 0; i_shuff < n_shufflings; i_shuff++)
	{
		fprintf(stderr, "Generating %d. shuffled merged regions.        \r", i_shuff);

		// Shuffle the indices.
		vector<int>* cur_shuff_indices = rng->fast_permute_indices(0, pooled_regions->size());

		shuffled_merged_regions_per_profile_per_shuff[i_shuff] = new vector<vector<t_annot_region*>*>();

		// Add all the regions for each profile.
		int cumul_reg_i = 0;
		for(int i_p = 0; i_p < (int)regions_per_profile->size(); i_p++)
		{
			//shuffled_merged_regions_per_profile_per_shuff[i_shuff]->push_back(new vector<t_annot_region*>());
			vector<t_annot_region*>* cur_profile_regions = new vector<t_annot_region*>();
			int n_regions_per_cur_track = (int)regions_per_profile->at(i_p)->size();

			// Add the regions from the shuffled list of indices.
			for(int i_reg = 0; i_reg < n_regions_per_cur_track; i_reg++)
			{
				cur_profile_regions->push_back(pooled_regions->at(cur_shuff_indices->at(cumul_reg_i)));

				// Update the current cumulative region index.
				cumul_reg_i++;
			} // i_reg loop.

			vector<t_annot_region*>* cur_profile_merged_regions = merge_annot_regions(cur_profile_regions, 1);

			delete cur_profile_regions;

			// Add the current list of regions.
			shuffled_merged_regions_per_profile_per_shuff[i_shuff]->push_back(cur_profile_merged_regions);
		} // i_p loop.

		delete cur_shuff_indices;
	} // i_shuff loop.

	delete rng;
	delete pooled_regions;
	return(shuffled_merged_regions_per_profile_per_shuff);
} // get_shuffled_merged_regions_per_profile


// Following should be moved to annot_region_tools.
...
{
	int n_profs = (int)ids_per_profile->size();

	if(n_profs != (int)per_profile_peak_regions->size())
	{
		fprintf(stderr, "The number of id's is not the same as numner of profile peak lists.\n");
		exit(0);
	}

	// setup the indices.
	int* prof_indices = new int[n_profs+2];
	for(int i_prof = 0; i_prof < n_profs; i_prof++)
	{
		prof_indices[i_prof] = i_prof;
	} // i_prof loop.

	// Setup the id list per aggregated signal peak.
	for(int i_agg_reg = 0; i_agg_reg < (int)aggregated_signal_peaks->size(); i_agg_reg++)
	{
		//vector<t_annot_region*>* cur_agg_peak_overlapping_peaks = new vector<t_annot_region*>();
		t_pooled_peak_association_info* assoc_info = new t_pooled_peak_association_info();
		assoc_info->agg_peak_oling_peaks = new vector<t_annot_region*>();
		assoc_info->oling_peaks_counts = new int[ids_per_profile->size() + 2];
		for(int i = 0; i < (int)ids_per_profile->size(); i++)
		{
			// Initialize the count of ith profile.
			assoc_info->oling_peaks_counts[i] = 0;
		} // i loop.

		aggregated_signal_peaks->at(i_agg_reg)->data = assoc_info;
	} // i_agg_reg loop.

	// Assign the profile index to all peaks.
	for(int i_prof = 0; i_prof < n_profs; i_prof++)
	{
		// Set the id's index for all the peaks for this profile.
		for(int i_p = 0; i_p < (int)per_profile_peak_regions->at(i_prof)->size(); i_p++)
		{
			per_profile_peak_regions->at(i_prof)->at(i_p)->data = (void*)(&(prof_indices[i_prof]));
		} // i_p loop.
	} // i_prof loop.

	// Overlap each aggregate peak with the per profile peaks and update the overlapping peak id's.
	for(int i_prof = 0; i_prof < (int)ids_per_profile->size(); i_prof++)
	{
		fprintf(stderr, "Overlapping peaks of profile %s with the aggregate profile peaks.\n", ids_per_profile->at(i_prof));
		vector<t_annot_region*>* intersected_regions = intersect_annot_regions(aggregated_signal_peaks, per_profile_peak_regions->at(i_prof), true);
		for(int i_int_reg = 0; i_int_reg < (int)intersected_regions->size(); i_int_reg++)
		{
			t_intersect_info* cur_intersect_info = (t_intersect_info*)(intersected_regions->at(i_int_reg)->data);
			t_annot_region* cur_oling_agg_peak = cur_intersect_info->src_reg;
			t_annot_region* cur_oling_prof_peak = cur_intersect_info->dest_reg;

			//vector<t_annot_region*>* agg_peak_oling_peaks = (vector<t_annot_region*>*)(cur_oling_agg_peak->data);
			t_pooled_peak_association_info* cur_agg_peak_assoc_info = (t_pooled_peak_association_info*)(cur_oling_agg_peak->data);
			cur_agg_peak_assoc_info->agg_peak_oling_peaks->push_back(cur_oling_prof_peak);

			// Update the count.
			int cur_oling_prof_i = ((int*)(cur_oling_prof_peak->data))[0];
			cur_agg_peak_assoc_info->oling_peaks_counts[cur_oling_prof_i]++;
		} // i_int_reg loop.
	} // i_prof loop.

	fprintf(stderr, "Allocating pairwise association count matrix.\n");
	double** pw_association_counts = new double*[n_profs + 2];
	for(int i_prof1 = 0; i_prof1 < n_profs; i_prof1++)
	{
		pw_association_counts[i_prof1] = new double[n_profs + 2];
		for(int i_prof2 = 0; i_prof2 < n_profs; i_prof2++)
		{
			pw_association_counts[i_prof1][i_prof2] = 0.0;
		} // i_prof2 loop.
	} // i_prof1 loop.

	// Build the association rules: Go over all the aggregate peaks, for each aggregate peak.
	fprintf(stderr, "Updating pairwise association count matrix.\n");
	for(int i_agg_reg = 0; i_agg_reg < (int)aggregated_signal_peaks->size(); i_agg_reg++)
	{
		// For the current aggregated peak, update all the pairwise associations.
		t_pooled_peak_association_info* cur_agg_peak_assoc_info = (t_pooled_peak_association_info*)(aggregated_signal_peaks->at(i_agg_reg)->data);
		vector<t_annot_region*>* associated_peaks_per_cur_agg_peak = cur_agg_peak_assoc_info->agg_peak_oling_peaks;

		for(int i_peak1 = 0; i_peak1 < (int)associated_peaks_per_cur_agg_peak->size(); i_peak1++)
		{
			int* prof1_i_ptr = (int*)(associated_peaks_per_cur_agg_peak->at(i_peak1)->data);
			int prof1_i = prof1_i_ptr[0];

			for(int i_peak2 = 0; i_peak2 < (int)associated_peaks_per_cur_agg_peak->size(); i_peak2++)
			{
				int* prof2_i_ptr = (int*)(associated_peaks_per_cur_agg_peak->at(i_peak2)->data);
				int prof2_i = prof2_i_ptr[0];

				if(i_peak1 != i_peak2)
				{
					// Update the current pairwise association.
					pw_association_counts[prof1_i][prof2_i]++;
				}
			} // i_prof2 loop.
		} // i_prof1 loop.
	} // i_agg_reg loop.
	...
	// Clear the profile indices.
	delete [] prof_indices;

	return(pw_association_counts);
}
							...
							vector<double>* bases, vector<int>* stat_counts, double sim_step)
{
	...
	t_restr_annot_region_list* regs1_res = restructure_annot_regions(regs1);
	t_restr_annot_region_list* regs2_res = restructure_annot_regions(regs2);
...
vector<t_annot_region*>* load_narrowPeak(char* narrowPeak_fp)
{
	...
	FILE* f_np = NULL;
	if (t_string::ends_with(narrowPeak_fp, ".gz"))
	{
		char ungzip_cmd[1000];
		sprintf(ungzip_cmd, "gzip -cd %s", narrowPeak_fp);
#ifdef _WIN32
		f_np = _popen(ungzip_cmd, "r");
#else 
		f_np = popen(ungzip_cmd, "r");
#endif
	}
	else
	{
		f_np = open_f(narrowPeak_fp, "r");
	}
...
vector<double>* get_self_inter_region_distance_distribution(vector<t_annot_region*>* annot_regions1)
{
	...
	t_restr_annot_region_list* restr_regs1 = restructure_annot_regions(annot_regions1);
	...
	for(int i_chr1 = 0; i_chr1 < (int)restr_regs1->chr_ids->size(); i_chr1++)
	{
		...
		sort(cur_chr_regs1->begin(), cur_chr_regs1->end(), sort_regions);

		for(int i_reg1 = 1; i_reg1 < (int)(cur_chr_regs1->size()-1); i_reg1++)
		{
			double cur_reg_min_distance = MIN(fabs((double)(cur_chr_regs1->at(i_reg1)->start - cur_chr_regs1->at(i_reg1-1)->end)), 
												fabs((double)(cur_chr_regs1->at(i_reg1)->end - cur_chr_regs1->at(i_reg1+1)->start)));
	...
	vector<t_annot_region*>* annot_regions2)
{
	...
	t_restr_annot_region_list* restr_regs1 = restructure_annot_regions(annot_regions1);
	t_restr_annot_region_list* restr_regs2 = restructure_annot_regions(annot_regions2);
													...
													bool case_sensitive)
{
	...
	for (int i_reg1 = 0; i_reg1 < orig_annot_regions1->size(); i_reg1++)
	{
	...
	for (int i_reg2 = 0; i_reg2 < orig_annot_regions2->size(); i_reg2++)
	{
	...
	for (int i_reg2 = 0; i_reg2 < annot_regions2->size(); i_reg2++)
	{ 
	...
	for (int i_reg1 = 0; i_reg1 < annot_regions1->size(); i_reg1++)
	{
		if (i_reg1 % 10000 == 0)
		{
			fprintf(stderr, "@ %d/%d region.              \r", i_reg1, annot_regions1->size());
		}

		int i_reg2 = t_string::fast_search_string_per_prefix(annot_regions1->at(i_reg1)->name, reg2_sorted_names, 0, reg2_sorted_names->size());
		...
		while (i_reg2 < reg2_sorted_names->size() &&
				(t_string::compare_strings(annot_regions1->at(i_reg1)->name, reg2_sorted_names->at(i_reg2)) ||
...
vector<t_annot_region*>* intersect_regions_per_names_OBSOLETE(vector<t_annot_region*>* orig_annot_regions1,
	vector<t_annot_region*>* orig_annot_regions2,
	bool case_sensitive)
{
	// Copy regions and to data; this ensures that we do not sort original annot regions list 2.
	...
	for (int i_reg1 = 0; i_reg1 < annot_regions1->size(); i_reg1++)
	{
		...
		for (int i_reg2 = 0; i_reg2 < annot_regions2->size(); i_reg2++)
		{
			if (t_string::compare_strings(annot_regions1->at(i_reg1)->name, annot_regions2->at(i_reg2)->name))
			{
				t_annot_region* new_intersecting_region = duplicate_region(annot_regions1->at(i_reg1));
			...
			// If reg2 name passed reg1 name w/o matches, we cannot find a match any more, so break.
			if (!t_string::compare_strings(annot_regions1->at(i_reg1)->name, annot_regions2->at(i_reg2)->name) &&
				!t_string::sort_strings_per_prefix(annot_regions2->at(i_reg2)->name, annot_regions1->at(i_reg1)->name))
			{
				break;
			}
		} // i_reg2 loop.
