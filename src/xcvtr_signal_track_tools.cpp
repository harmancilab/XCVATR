...
//#include "xcvtr_full_histogram.h"
...
void quantile_normalize_signal_matrix_4th_col_signals(char* signal_regions_BED_fp, char* op_fp)
{
	...
	for (int i_reg = 0; i_reg < signal_node_regs->size(); i_reg++)
	{
	...
	for (int i_s = 0; i_s < n_loaded_samples; i_s++)
	{
		...
		for (int rank_i = 0; rank_i < per_sample_signal_nodes[i_s]->size(); rank_i++)
		{
	...
	double* per_rank_avg_signals = new double[signal_node_regs->size() + 2];
	for (int rank_i = 0; rank_i < signal_node_regs->size(); rank_i++)
	{
	...
	for (int col_i = 0; col_i < header_col_ids->size(); col_i++)
	{
		if (col_i > 0)
		{
...
if (__DUMP_SIGNAL_TRACK_MSGS__)
	...
	if(_l_bin == 0)
	{
		n_bins = all_target_regions->size() + 10;
	}
	else
	{
...
if(__DUMP_SIGNAL_TRACK_MSGS__)
{
	...
	if(quantified_regions != NULL &&
		quantified_regions->size() != n_bins)
	{
											...
											int& l_multitrack)	// Length of the track.
{
	...
	fprintf(stderr, "Generating multitrack signal profiles using %d read dirs, %d binary tracks, %d regions over %d target regions (%d bps) with bin length of %d and log base of %.4f.\n", 
			preprocessed_reads_dirs->size(), bin_track_dirs->size(), region_fps->size(), target_regions->size(), (int)target_covg, l_bin, log_base);

	int n_tracks = (int)(preprocessed_reads_dirs->size() + bin_track_dirs->size() + region_fps->size());
	...
	if(chr_ids_2_use == NULL)
	{
		if(target_regions != NULL)
		{
			...
			preprocessed_reads_dirs->size() > 0)
		{
...
#undef __MANUAL_CHECK__
			...
			for(int i_r = 0; i_r < per_track_quantified_regions[track_i]->size(); i_r++)
			{
														...
														vector<char*>* track_ids)
{
	...
	int n_tracks = reads_dirs->size() + track_bin_dirs->size() + region_fps->size();
