...
scored_variants_fp <- args[2];
variant_allele_counts_fp <- args[3];
metadata_fp <- args[4];

##################################################################################################################
#tsne_coords_fp <- '../../../Figure4_SC_Meningioma_SNV_CNV/Single_Cell/Front/SNV_Indel/Embedding/frontal_cell_tsne.txt';
#scored_variants_fp <- '../../../Figure3_Significant_Clumps_Overlaps_Disjoints/Darmanis_etal/BT_S2/Scored_Variants/global_tsne/filtered_summarized_vars.txt';
#variant_allele_counts_fp <- '../../../Figure4_SC_Meningioma_SNV_CNV/Single_Cell/Front/SNV_Indel/Counts/counts_dbsnp_filtered.txt_covg_filtered.txt_AF.txt';
#metadata_fp <- 'none';
##################################################################################################################

z_score_cutoff <- 2;
min_above_below_fraction <- 0.01;
min_n_above_cells <- 50;
max_above_AF_FE_pval <- -2;
max_exp_dist_weight <- 1; # Absolute maximum
min_exp_dist_weight <- 1/(10*10*10); # Absolute minimum
...
if(!file.exists(scored_variants_fp))
{
	stop(sprintf("Could not find the scored variants list @ %s", scored_variants_fp));
}
############################################################################################################
...
top_vars <- read.delim(scored_variants_fp, header=TRUE, stringsAsFactors = FALSE);
...
if(nrow(top_vars) == 0)
{
	stop("No scored variants, stopping");
}
...
plot_i <- 1;
pdf('embedding_SNV_Indel_AFs.pdf');
...
for(cur_uniq_var_i in 1:length(unique_var_names))
{
	...
	center_circle_df <- data.frame(r= min((1/cur_top_vars$exp_dist_weight)^.5, (cur_top_vars$effective_radius)^.5), x0=cur_top_vars$cur_sample_x, y0=cur_top_vars$cur_sample_y, alpha=0.0001, size=1);
...
#											colour = "grey"));													
	...
	p_hist <- ggplot(filt_AF_tsne_df, aes(x=alt_AF)) + 
	  ggtitle(sprintf("AF Distribution: %s", variant_loci[var_loci_i, 4]))+
	  geom_histogram(binwidth=0.05) + geom_density()+
	  xlab('Alternate Allele Frequency (AF)') + ylab('Frequency of Alternate AF') + 
